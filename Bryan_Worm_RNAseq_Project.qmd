...
```{r}
expression_data <- read.csv("cts.csv", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
...
write.csv(res_2, file = "DESeq_Results.csv", row.names=FALSE)
...
library(tidyverse)
library(biomaRt)
res_22 <- read_csv("DESeq_Results.csv")

mart <- useMart(host = "metazoa.ensembl.org",
                         biomart = "metazoa_mart",
                         dataset = "celegans_eg_gene")

bm <- getBM(attributes=c("ensembl_gene_id"), mart=mart) %>%
  distinct() %>%
  as_data_frame() %>%
  na.omit()
bm
